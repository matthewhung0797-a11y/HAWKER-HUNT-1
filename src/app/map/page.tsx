...
export default function MapPage() {
  ...
  useEffect(() => {
    ...
    avatarImg.src = "/ui/player-marker-hd.png";
    avatarImg.alt = "";
    avatarImg.draggable = false;
    // 圖形長寬比 ≈0.478（直身角色）：高 58px、闊 28px、貼底置中；源圖 1024px 高，最大 4x 縮放＋3x DPR 仍清晰
    ...
    const syncWorldScale = () => {
      ...
      // 據點標記固定 60px（badgeScalers 唔寫 transform＝永遠原大小）；定位標記跟地圖縮放、保持精靈 2 倍大小
      const ps = playerScalerRef.current;
      if (ps) ps.style.transform = `scale(${(sp).toFixed(3)})`;
    };
